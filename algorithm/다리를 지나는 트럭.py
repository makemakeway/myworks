...
def solution(bridge_length, weight, truck_weights):
    ...
    while truck_on_bridge:
        ...
        # 건너려고 대기중인 트럭이 있을 경우
        if len(truck_waiting) > 0:
            if bridge_weight + truck_waiting[0] <= weight:
                bridge_weight += truck_waiting[0]
                truck_on_bridge.append(truck_waiting.popleft())
            else:
